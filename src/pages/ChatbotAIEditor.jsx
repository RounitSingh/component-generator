...
//             {/* Chat Panel */}
//             <div className="md:col-span-3 max-w-auto p-6 flex flex-col rounded-2xl border-gray-200 drop-shadow-lg max-h-screen overflow-hidden bg-white relative">
//                 {/* Header */}
...
//                         <input
//                             type="text"
//                             className="input flex-1 border border-yellow-500 relativ"
...
//                         </button>
//                     </div> */}
//                     <div className="flex gap-2 items-center flex-wrap relative w-full">
//                         <div className="relative flex-1">
//                             <input
//                                 type="text"
//                                 className="input w-full  pr-10 rounded-md"
//                                 placeholder={
...
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { duotoneSpace } from 'react-syntax-highlighter/dist/esm/styles/prism';
import { Download, Copy, Send, Image as ImageIcon, Loader2, AlertCircle, StarsIcon, Edit3, X, ArrowDown, Plus } from 'lucide-react';
import useChatbotChatStore from '../store/chatbotChatStore';
import useChatbotComponentStore from '../store/chatbotComponentStore';
import { generateComponentWithGemini } from '../utils/geminiApi';
import DynamicPreview from '../components/DynamicPreview';
                            ...
                            <div className="flex items-center gap-3">
                                {code.jsx && code.css && (
                                    <div className="bg-white/20 backdrop-blur-sm rounded-full px-3 py-1.5 text-xs font-medium">
                                        v{useChatbotComponentStore.getState().components?.filter(c => c.isCurrent).length || 1}
                                    </div>
                                )}
                                <button
                                    onClick={handleClearChat}
                                    ...
                                    <div className={`flex flex-col max-w-[85%] ${msg.type === 'prompt' ? 'items-start' : 'items-end'}`}>
                                        ...
                                        {msg.type === 'prompt' && msg.selectedTarget && (
                                            <div className="mt-2 flex items-center gap-2 bg-indigo-50 text-indigo-700 border border-indigo-100 drop-shadow-xs rounded-full px-3 py-1.5 text-xs font-medium">
                                                ...
                                                <span className="text-xs font-mono">Target: <span className="font-mono text-xs">{msg.selectedTarget}</span></span>
                                            </div>
                                        )}
                                        {msg.type === 'prompt' && (() => {
                                            const { components } = useChatbotComponentStore.getState();
